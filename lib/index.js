...
FBCache.update = async (dbms, route, data, id) => {
    if(!route)
        throw new Error("route can't be null or undefined");
    if(!data || Object.entries(data).length === 0 || data === [])
        throw new Error("data can't be null or undefined");
...
FBCache.delete = async (dbms, route, id) => {
    if(!route)
        throw new Error("route can't be null or undefined");
    if(!id)
        throw new Error("id can't be null or undefined");
...
function FBCacheR() {
    ...
    this.ref = (route) => {
        if(this.dbms === service.REAL_TIME && route)
            this.route = route;
        return this;
    },
    ...
    this.once = async () => {
        if(this.dbms && this.route){
            return await FBCache.get(this.dbms, this.route);
        }
    },
    ...
    this.set = async (data) => {
        if(this.dbms === service.REAL_TIME && this.route && this.id && data){
            return await FBCache.insert(this.dbms, this.route, data, this.id);
        }
    },

    this.push = async (data) => {
        if(this.dbms === service.REAL_TIME && this.route){
            return await FBCache.insert(this.dbms, this.route, data);
        }
    },

    this.update = async (data) => {
        if(this.dbms && this.route && this.id)
            return await FBCache.update(this.dbms, this.route, data, this.id);
    }

    this.remove = async () => {
        if(this.dbms === service.REAL_TIME && this.route && this.id){
            return await FBCache.delete(this.dbms, this.route, this.id);
        }
    }
